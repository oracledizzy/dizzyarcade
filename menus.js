...
Object.assign(ArcadeApp.prototype, {
  ...
  make_play_menu_item(text, command, theme, cy, opts={}) {
    ...
    const w = starred ? 260 : 220, h = 46, cx = 300;
    const idx = this.menu_items.length;
    ...
    if (starred) {
      const bx = cx + w/2 - 20, by = cy;
      this.canvas.create_oval(bx-14, by-14, bx+14, by+14,
        { fill:theme.bg, outline:GREEN, width:2, tags:[tag] });
      this.draw_level_icon(bx, by, level_id, tag, 2);
    }
  },
  ...
  show_play_menu() {
    ...
    if (this.unlocked_levels.includes('esoteric_snake')) {
      ...
      this.make_play_menu_item('SNAKE', () => this.show_snake_mode_select(), theme, y,
        { starred:true, level_id:'esoteric_snake', high_score:best });
    } else {
      this.make_play_menu_item('SNAKE GAME', () => this.show_snake_type_select(), theme, y,
    ...
    if (this.unlocked_levels.includes('flappy_alt_dimension')) {
      ...
      this.make_play_menu_item('FLAPPY BIRD', () => this.show_flappy_mode_select(), theme, y,
        { starred:true, level_id:'flappy_alt_dimension', high_score:best });
    } else {
      this.make_play_menu_item('FLAPPY BIRD', () => this.start_flappy(), theme, y,
        { high_score: hs.FLAPPY ?? 0 });
    }
    y += 58;

    this.make_play_menu_item('PONG (2 PLAYER)', () => this.prep_pong(), theme, y, { high_score: hs.PONG ?? 0 }); y += 58;
    this.make_play_menu_item('SPACE INVADERS', () => this.start_space_invaders(), theme, y, { high_score: hs.SPACE_INVADERS ?? 0 }); y += 58;
    this.make_play_menu_item('SWORD ARENA', () => this.start_sword_arena(), theme, y, { high_score: this.arena_best_wave }); y += 58;
    this.make_play_menu_item('BLACKJACK', () => this.start_blackjack_betting(), theme, y, { high_score: hs.BLACKJACK ?? 0 }); y += 58;
  ...
  show_snake_mode_select() {
    ...
    this.build_mode_select_box(180, 300, 220, 240, 'SNAKE',
  ...
  show_snake_type_select() {
    ...
    this.build_mode_select_box(180, 300, 220, 240, 'INFINITE',
      `Best: ${this.format_tokens(this.high_scores.SNAKE ?? 0)}`,
      (cx,cy,tag) => this.draw_snake_icon(cx, cy, [GREEN, theme.secondary], tag),
      () => this.start_snake(), theme);

    this.build_mode_select_box(420, 300, 220, 240, 'LEVELS',
    ...
    this.make_menu_item('< BACK', back, theme, 300, 470, 200, 36);
    this.menu_active = true;
    this.menu_selected_index = 0;
    this.refresh_menu_highlight();
  },
